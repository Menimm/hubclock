...
- מסך השעון מציג כעת את מזהה המכשיר הפעיל ומאפשר להעתיק אותו בקליק, כדי לסייע בבדיקת התאמת מכשירים.
- דוח היומן היומי מציג מזהי מכשיר קריאים גם עבור רשומות ללא מזהה או עם מזהה "unknown-device".
- רשימת העובדים שבמשמרת אינה מציגה עוד מספרי ת"ז, ומזהה המכשיר הוזז לעמודה השמאלית כדי לצמצם חשיפה לנתוני זיהוי.
- במסך הדוחות נוסף מתג שמאפשר לכלול או להסתיר מזהי מכשירים גם בייצוא לאקסל, למקרי שימוש רגישים לפרטיות (המתג מופיע גם אם ההגדרה המרכזית מכובה, אך יישאר מושבת).
- הגדרת "הצגת מזהי מכשיר" במסך ההגדרות קובעת כעת אם מזהים יוצגו במסך השעון, בדוחות החיים ובייצוא הקבצים (כולל כיבוי מוחלט כברירת מחדל).
- כותרת מסך השעון מציגה את תאריך היום בפורמט DD/MM/YY, ורשימת העובדים הפעילים מציגה רק את שעת הכניסה (ללא תאריך) לשמירה על פרטיות.
- שדה קוד העובד במסך השעון כעת משתמש במקלדת מספרית ייעודית במכשירי מובייל ומנקה תווים שאינם ספרות, כדי להבטיח קלט מספרי בלבד.
