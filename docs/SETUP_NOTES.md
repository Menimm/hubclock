...
- שינוי פורט שרת הפיתוח נעשה דרך `VITE_DEV_PORT` ב-`frontend/.env`. לאחר שינוי, הריצו שוב `sudo ./scripts/install_services.sh --dev` או הקפידו להריץ `npm run dev` מתוך תיקיית `frontend/` עם הפרמטר `--port` המתאים.
